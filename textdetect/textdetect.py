...
class DetectedTextBox:
    ...
    def setHeight(self,coords_list):
        self.height = max(coords_list)
    
    def setWidth(self,coords_list):
        self.width = min(coords_list)
    
    def setCoords(self,coords_list):
        self.startX = coords_list[0]
        self.startY = coords_list[1]
        self.endX = coords_list[2]
        self.endY = coords_list[3]
...
def text_detect(imagebytes):
    ...
    for i in np.arange(box_num):
        ...
        dbox = DetectedTextBox()
        dbox.setHeight([startY[i],endY[i]])#, startX[i],endX[i])
        dbox.setWidth([startX[i],endX[i]])#, startX[i],endX[i])
        #print("coords",startY[i],endY[i], startX[i],endX[i])
        print("coords", startX[i],startY[i],endX[i],endY[i])
        #dbox.setCoords([startX[i],startY[i],endX[i],endY[i]])
        #y_hights.append(startY[i])#,endY[i], startX[i],endX[i])
        ...
        print("dbg line {} : {} ({})".format(i, text, final_conf[i]), flush=True)
        text_info = {}
        text_info['conf'] = final_conf[i]
        text_info['text'] = text
        dbox.text = text

        BoxesList.append(dbox)
    ...
    height_difference_to_be_the_same_line = 10

    for dbox in BoxesList:
        print("dbox ",dbox.text,dbox.height)
        try:
            printable = set(string.printable)
            text = ''.join(filter(lambda x: x in printable, dbox.text))

            #code_part = int(dbox.text)
            code_part = int(text)
            print("code part!",code_part,len(str(code_part)))
            codes_list.append(code_part)

            assert len(str(code_part)) <= 3

            print(codes_dict)
            print(codes_dict.keys())
            print(dbox.height in codes_dict.keys())
            added_to_dict = False
            for height in codes_dict.keys():
                difference = abs(height - dbox.height)
                if difference <= height_difference_to_be_the_same_line:
                    # add to start or end of list
                    if widths_dict[height] < dbox.width:
                        codes_dict[height].append(code_part)
                    else:
                        codes_dict[height].insert(0,code_part)
                    print("adding to height ",dbox.height)
                    added_to_dict = True

            if not added_to_dict:
                print("adding new height ",dbox.height)
                codes_dict[dbox.height] = []
                codes_dict[dbox.height].append(code_part)
                # track width as well
                widths_dict[dbox.height] = dbox.width
